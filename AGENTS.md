...
`npx` кэширует sandbox в `~/.npm/_npx/<hash>/` и **не обновляет git-зависимость** при каждом reload MCP — для всех ref выше. После релиза или смены ref: `npx clear-npx-cache`, reload MCP.
