...
MCP-сервер: Jira, Confluence, Insight (Assets), Zephyr Scale. Точка входа — `src/index.ts` (регистрация tools), после правок: `npm run build`, **поднять `version` в `package.json`**, push в `master` (CI → `dist` + git-тег `v{version}`) и reload MCP в Cursor.
...
Релиз: bump `version` в `package.json` → push `master` → CI ставит тег `v{version}` на коммит `dist`.
...
Тесты: `npm test`. После изменений в `src/` — build + точечный тест в `tests/tools/` или `tests/src/`.
