...
server.tool("inspect-zephyr-project", "PRIMARY for test-wdio → Zephyr setup: inspect an unfamiliar Zephyr project before creating cases. Returns observed custom field values, folder examples, and sample cases. Use projectKey from test-wdio --qaseProject.", inspectZephyrProjectSchema, inspectZephyrProjectHandler(zephyr, jiraConfig));
server.tool("upsert-zephyr-testcase", "PRIMARY for test-wdio → Zephyr sync after adding or changing wdio tests. Pass wdioItTitle (full it() string), precondition, testScriptPlainText. If title ends with #PREFIX-Tnnn → update name/steps/precondition; otherwise create a new case and return the key to append to it(). On create, pass projectKey (--qaseProject) or inheritCustomFieldsFrom a sibling case.", upsertZephyrTestCaseSchema, upsertZephyrTestCaseHandler(zephyr, jiraConfig));
server.tool("get-zephyr-testcase", "Read one Zephyr case before updating or to compare with a changed test-wdio spec. For bulk sync prefer upsert-zephyr-testcase.", getZephyrTestCaseSchema, getZephyrTestCaseHandler(zephyr, jiraConfig));
server.tool("search-zephyr-testcases", "Find Zephyr cases by projectKey/text when the #PREFIX-Tnnn key is unknown or you need a reference case for inheritCustomFieldsFrom.", searchZephyrTestCasesSchema, searchZephyrTestCasesHandler(zephyr, jiraConfig));
...
server.tool("update-zephyr-testcase", "Low-level update by key. Prefer upsert-zephyr-testcase with wdioItTitle or testCaseKeyOrUrl for test-wdio workflows.", updateZephyrTestCaseSchema, updateZephyrTestCaseHandler(zephyr, jiraConfig));
server.tool("delete-zephyr-testcase", "Permanently delete a Zephyr test case by key or URL. Requires confirm=true. Use when a case is obsolete (e.g. replaced in test-wdio by another key). Call get-zephyr-testcase first to verify the target.", deleteZephyrTestCaseSchema, deleteZephyrTestCaseHandler(zephyr, jiraConfig));
...
server.tool("send-zephyr-test-result", "Record Pass/Fail/Blocked/Not Executed result for a testcase inside a Zephyr test run.", sendZephyrTestResultSchema, sendZephyrTestResultHandler(zephyr, jiraConfig));
