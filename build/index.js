...
import { withZephyrNotes, ZEPHYR_PROJECT_KEY_NOTE, ZEPHYR_REST_ONLY_NOTE, } from "./utils/zephyr-tool-notes.js";
// Initialize clients
...
server.tool("inspect-zephyr-project", withZephyrNotes("PRIMARY for test-wdio → Zephyr setup: inspect an unfamiliar Zephyr project before creating cases. Returns observed custom field values, folder examples, and sample cases. Use projectKey from test-wdio --qaseProject.", ZEPHYR_REST_ONLY_NOTE, ZEPHYR_PROJECT_KEY_NOTE), inspectZephyrProjectSchema, inspectZephyrProjectHandler(zephyr, jiraConfig));
server.tool("upsert-zephyr-testcase", withZephyrNotes("PRIMARY for test-wdio → Zephyr sync after adding or changing wdio tests. Pass wdioItTitle (full it() string), precondition, testScriptPlainText. If title ends with #PREFIX-Tnnn → update name/steps/precondition; otherwise create a new case and return the key to append to it(). On create, pass projectKey (--qaseProject) or inheritCustomFieldsFrom a sibling case.", ZEPHYR_REST_ONLY_NOTE), upsertZephyrTestCaseSchema, upsertZephyrTestCaseHandler(zephyr, jiraConfig));
server.tool("get-zephyr-testcase", withZephyrNotes("Read one Zephyr case before updating or to compare with a changed test-wdio spec. For bulk sync prefer upsert-zephyr-testcase. Accepts test case key or Tests.jspa #/testCase/KEY URL (URL is parsed for the key only — do not fetch the URL).", ZEPHYR_REST_ONLY_NOTE), getZephyrTestCaseSchema, getZephyrTestCaseHandler(zephyr, jiraConfig));
server.tool("search-zephyr-testcases", withZephyrNotes("Find Zephyr cases by projectKey when the #PREFIX-Tnnn key is unknown or you need a reference case for inheritCustomFieldsFrom.", ZEPHYR_REST_ONLY_NOTE, ZEPHYR_PROJECT_KEY_NOTE), searchZephyrTestCasesSchema, searchZephyrTestCasesHandler(zephyr, jiraConfig));
server.tool("create-zephyr-testcase", withZephyrNotes("Low-level create. Prefer upsert-zephyr-testcase for test-wdio workflows.", ZEPHYR_REST_ONLY_NOTE), createZephyrTestCaseSchema, createZephyrTestCaseHandler(zephyr, jiraConfig));
server.tool("update-zephyr-testcase", withZephyrNotes("Low-level update by key. Prefer upsert-zephyr-testcase with wdioItTitle or testCaseKeyOrUrl for test-wdio workflows.", ZEPHYR_REST_ONLY_NOTE), updateZephyrTestCaseSchema, updateZephyrTestCaseHandler(zephyr, jiraConfig));
server.tool("delete-zephyr-testcase", withZephyrNotes("Permanently delete a Zephyr test case by key or URL. Requires confirm=true. Use when a case is obsolete (e.g. replaced in test-wdio by another key). Call get-zephyr-testcase first to verify the target.", ZEPHYR_REST_ONLY_NOTE), deleteZephyrTestCaseSchema, deleteZephyrTestCaseHandler(zephyr, jiraConfig));
server.tool("create-zephyr-testrun", withZephyrNotes("Create a Zephyr Scale test run for a project.", ZEPHYR_REST_ONLY_NOTE), createZephyrTestRunSchema, createZephyrTestRunHandler(zephyr, jiraConfig));
server.tool("send-zephyr-test-result", withZephyrNotes("Record Pass/Fail/Blocked/Not Executed result for a testcase inside a Zephyr test run.", ZEPHYR_REST_ONLY_NOTE), sendZephyrTestResultSchema, sendZephyrTestResultHandler(zephyr, jiraConfig));
