...
import {
    ...
    fetchReferenceCustomFields,
    formatUpsertResult,
    mergeUpsertName,
    parseWdioItTitle,
    resolveUpsertSteps,
    zephyrUpsertFieldsSchema,
} from "../../utils/zephyr-wdio-sync.js";
...
export const upsertZephyrTestCaseSchema = {
    ...
    projectKey: z
        ...
        .optional()
        .describe(
            "Required when creating a new case (no #PREFIX-Tnnn in wdioItTitle). Usually equals test-wdio --qaseProject.",
        ),
    testCaseKeyOrUrl: z
        .string()
        .optional()
        .describe(
            "Explicit case key or URL. Overrides #suffix parsed from wdioItTitle. Use for updates when you only have the key.",
        ),
    inheritCustomFieldsFrom: z
        .string()
        .optional()
        .describe(
            "Existing case key in the same project. On create, copies its customFields when customFields are not provided.",
        ),
    automationStatus: z
        .enum(["automated", "is-not-automated", "to-be-automated"])
        .optional()
        .describe(
            "Shortcut for Automation status custom field on create. Prefer is-not-automated until wdio reports results to Zephyr.",
        ),
};
...
type UpsertInput = {
    ...
    inheritCustomFieldsFrom?: string;
    automationStatus?: "automated" | "is-not-automated" | "to-be-automated";
    name?: string;
    wdioItTitle?: string;
    objective?: string;
    precondition?: string;
...
const resolveTargetKey = (input: UpsertInput): string | undefined => {
...
const resolveCustomFieldsForCreate = async (
    zephyr: AxiosInstance,
    projectKey: string,
    input: UpsertInput,
    ...
    if (
        input.inheritCustomFieldsFrom &&
        input.inheritCustomFieldsFrom.trim().length > 0
    ) {
        const inherited = await fetchReferenceCustomFields(
            zephyr,
            extractZephyrTestCaseKey(input.inheritCustomFieldsFrom),
        );
        if (inherited) {
            return inherited;
        }
    ...
    const searchResponse = await zephyr.post<ZephyrTestCaseSummary[]>(
        "/testcase/search",
        {
    ...
    const inspection = collectProjectInspection(
        projectKey,
        searchResponse.data,
    );
...
export const upsertZephyrTestCaseHandler =
    ...
    async (input: UpsertInput) => {
        ...
        try {
            ...
            if (targetKey) {
                ...
                const updatedResponse = await zephyr.get<ZephyrTestCase>(
                    `/testcase/${targetKey}`,
                );
            ...
            const customFields = await resolveCustomFieldsForCreate(
                zephyr,
                input.projectKey,
                input,
            );
            ...
            const createdResponse = createdKey
                ? await zephyr.get<ZephyrTestCase>(`/testcase/${createdKey}`)
                : response;
