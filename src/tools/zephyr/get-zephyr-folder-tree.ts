...
import {
    formatAxiosError,
    formatFolderTree,
    ZephyrFolderTreeRoot,
    ZephyrProjectSummary,
...
export const getZephyrFolderTreeSchema = {
    project: z
        .string()
        .describe(
            'Project key (e.g. "GRW"), numeric projectId, or a Tests.jspa URL containing projectId=NNNNN.',
        ),
};
...
const resolveProjectId = async (
    ...
    const response =
        await zephyrTests.get<ZephyrProjectSummary[]>("/project");
    const projects = Array.isArray(response.data) ? response.data : [];
    const match = projects.find(
        (p) => p.key?.toUpperCase() === trimmed.toUpperCase(),
    );
    if (!match) {
        const keys = projects
            .map((p) => p.key)
            ...
            .join(", ");
        throw new Error(
            `Project "${trimmed}" not found. Pass a numeric projectId or a known key. Available keys: ${keys}`,
        );
    }
...
export const getZephyrFolderTreeHandler =
    (zephyrTests: AxiosInstance, jiraConfig: JiraConfig) =>
    async ({ project }: { project: string }) => {
        ...
        try {
            ...
            const response = await zephyrTests.get<ZephyrFolderTreeRoot>(
                `/project/${id}/foldertree/testcase`,
            );
