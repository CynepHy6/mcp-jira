...
export const searchInsightAssetsSchema = {
    iql: z
        .string()
        .describe(
            'Insight IQL query, e.g. Key = "INFRA-1097573", Name = "Growth", objectType = Team',
        ),
    maxResults: z
        .number()
        .default(20)
...
export const searchInsightAssetsHandler =
    (insight: AxiosInstance, jiraConfig: JiraConfig) =>
    async ({ iql, maxResults }: { iql: string; maxResults: number }) => {
        ...
        try {
            const response = await insight.get<InsightSearchResponse>(
                "/iql/objects",
                {
                    params: {
                        iql,
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: formatInsightSearchResults(
                            response.data,
                            jiraConfig.host,
                            `IQL: ${iql}`,
                        ),
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Insight search failed: ${(error as Error).message}`,
                    },
                ],
            };
        }
