...
import {
    ZephyrTestCase,
    ZephyrTestCaseSummary,
    ZephyrTestStep,
} from "./zephyr-utils.js";
...
export const zephyrUpsertFieldsSchema = {
    name: z
        ...
        .optional()
        .describe(
            "Test case title in Zephyr. If omitted with wdioItTitle, parsed from it() without #KEY suffix.",
        ),
    wdioItTitle: z
        .string()
        .optional()
        .describe(
            'Full test-wdio it() title. Example: "Payment banner is visible #PROJ-T123". If it ends with #PREFIX-Tnnn → update that case; otherwise create a new one.',
        ),
    objective: z
        .string()
        .optional()
    ...
    precondition: z
        ...
        .optional()
        .describe(
            "Preconditions. Prefer multiline numbered list; HTML <br /> is allowed.",
        ),
    testScriptPlainText: z
        .string()
        .optional()
        .describe(
            "Manual-style steps for Zephyr (multiline). Use blocks like \"Шаг 1: ...\\n\\nОжидаемый результат: ...\". Converted to STEP_BY_STEP script.",
        ),
    steps: z
        .array(zephyrStepSchema)
        .optional()
        .describe(
            "Structured step-by-step script. Prefer testScriptPlainText for test-wdio sync unless you already have structured steps.",
        ),
    ...
    customFields: z
        ...
        .optional()
        .describe(
            "Project-specific custom fields. On create, use inspect-zephyr-project or inheritCustomFieldsFrom when unsure.",
        ),
};

export const hasZephyrScriptInput = (input: {
    testScriptPlainText?: string;
    steps?: ZephyrTestStep[];
}): boolean =>
    Boolean(
        (input.testScriptPlainText &&
            input.testScriptPlainText.trim().length > 0) ||
            (input.steps && input.steps.length > 0),
    );

export const mergeUpsertName = (
    explicitName: string | undefined,
    wdioItTitle: string | undefined,
): string => {
...
export const parseWdioItTitle = (
    title: string,
): { testCaseKey?: string; name: string } => {
...
export const formatPlainTextAsZephyrSteps = (
    plainText: string,
): ZephyrTestStep[] => {
...
export interface ZephyrProjectInspection {
    projectKey: string;
    sampleCases: ZephyrTestCaseSummary[];
    customFieldOptions: Record<string, string[]>;
    folderExamples: string[];
    automationStatusValues: string[];
}

export const collectProjectInspection = (
    projectKey: string,
    cases: ZephyrTestCaseSummary[],
): ZephyrProjectInspection => {
    const customFieldOptions: Record<string, Set<string>> = {};
    const folderExamples = new Set<string>();
    ...
    for (const testCase of cases) {
        ...
        for (const [field, value] of Object.entries(testCase.customFields)) {
            if (!customFieldOptions[field]) {
                customFieldOptions[field] = new Set<string>();
            }
...
export const formatProjectInspection = (
    inspection: ZephyrProjectInspection,
    host: string,
): string => {
    const lines = [
    ...
    if (fieldNames.length === 0) {
        ...
        for (const field of fieldNames.sort()) {
            lines.push(
                `- **${field}:** ${inspection.customFieldOptions[field].join(" | ")}`,
            );
        }
    }
    ...
    if (inspection.sampleCases.length === 0) {
        ...
        for (const testCase of inspection.sampleCases) {
            lines.push(
                `- ${testCase.key}: ${testCase.name ?? "Untitled"}`,
                `  URL: ${host.includes("://") ? host : `https://${host}`}/secure/Tests.jspa#/testCase/${testCase.key}`,
            );
        }
    }
    ...
    lines.push(
        "",
        "## test-wdio sync checklist",
        "1. Call upsert-zephyr-testcase with wdioItTitle + precondition + testScriptPlainText.",
        "2. If wdioItTitle ends with #PREFIX-Tnnn → existing case is updated.",
        "3. If there is no #suffix → a new case is created; append returned `#KEY-Tnnn` to the it() title.",
        "4. Match Automation status to reality: automated only when the wdio test already reports results to Zephyr.",
    );

    return lines.join("\n");
};

export const formatUpsertResult = ({
    action,
    testCase,
    host,
    wdioItTitle,
    ...
    if (action === "created") {
        const name =
            wdioItTitle !== undefined
                ? parseWdioItTitle(wdioItTitle).name
                : testCase.name;
        lines.push(
            "",
            "Next step in test-wdio:",
            `\`it('${name} #${key}', async () => { ... });\``,
        );
    }
    ...
    if (testCase.testScript?.steps && testCase.testScript.steps.length > 0) {
        ...
        testCase.testScript.steps.forEach((step, index) => {
            lines.push(
                `${index + 1}. ${step.description.replace(/<br \/>/g, "\n   ")}`,
            );
        });
    }
...
export const buildDefaultCustomFields = (
    inspection: ZephyrProjectInspection,
...
export const fetchReferenceCustomFields = async (
    zephyr: { get: (url: string) => Promise<{ data: ZephyrTestCase }> },
    referenceKey: string,
