...
Для тестирования и отладки инструментов доступен удобный shell-скрипт:

### Универсальное тестирование

```bash
# Синтаксис
./test-tool.sh <tool-name> '<json-arguments>'

# Примеры использования
./test-tool.sh read-description '{"issueKey": "VIM-27658"}'
...
- **Автоматическая сборка**: Скрипт автоматически собирает проект перед тестированием
- **Подробный вывод**: Показывает результат работы инструмента в JSON формате
- **Обработка ошибок**: Отображает детальную информацию об ошибках подключения или API
- **Поддержка всех инструментов**: Работает с любыми MCP инструментами проекта

**Примечание**: Убедитесь, что настроены переменные окружения в `.env` файле перед тестированием.
